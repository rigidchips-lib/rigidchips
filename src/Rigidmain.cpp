...
static double s_FPS;
//現在の制限FPS
static int s_LimitFPS = 30;

static GFloat s_TotalPower;

//モデル読み込みからのフレーム数
static int s_TickCount;
//起動からのフレーム数
static int s_SystemTickCount;
	...
	if (code == NC_ERR) {
		...
		for (int i = 0;i < g_DPlay->GetMaxPlayers();i++) {
			if (g_PlayerData[i].ReceiveData.info.dpnidPlayer == playerInfo->dpnidPlayer) {
				...
				if (scenarioCode == s_RecieaveMessageCode[i]) {
					char *str = (char*)&data[4];
					strcpy(s_RecieaveMessageData[i], str);
				}
			}
		}
	}
		...
		if (NetworkDlg) {
			...
			g_DPlay->SendTo(playerInfo->dpnidPlayer, (BYTE*)&strm2, size, 180, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE);
		}
	}
		...
		if (NetworkDlg) {
			...
			strm2.code = 1;
			char *str = (char*)strm2.data;
			if (g_World->NetStop) sprintf(str, "s_FPS=Pause (Base=%d) ", s_LimitFPS);
			else sprintf(str, "s_FPS=%.1f (Base=%d) ", s_FPS, s_LimitFPS);
			DWORD size = strlen(str) + 1 + sizeof(short);
			g_DPlay->SendTo(playerInfo->dpnidPlayer, (BYTE*)&strm2, size, 180, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE);
		}
	}
...
int CALLBACK DlgNetworkProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
				...
				if (str1[0] == '&') {
					...
					if (id >= 0) {
						if (g_PlayerData[id].ReceiveData.info.dpnidPlayer != g_DPlay->GetLocalPlayerDPNID()) {
							g_DPlay->SendTo(g_PlayerData[id].ReceiveData.info.dpnidPlayer, (BYTE*)&stream, (DWORD)size, 180, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE);
						}
					}
					else g_DPlay->SendTo(DPNID_ALL_PLAYERS_GROUP, (BYTE*)&stream, (DWORD)size, 180, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE);
					...
					g_DPlay->SendTo(DPNID_ALL_PLAYERS_GROUP, (BYTE*)&stream, size, 2000, DPNSEND_NOLOOPBACK | DPNSEND_GUARANTEED | DPNSEND_PRIORITY_HIGH);
		...
		case IDC_STARTBUTTON:
		{
			...
			g_MyPlayerData.base_fps = (s_LimitFPS << 16) + 30;
	...
	for (int i = 0;i < size;i++) {
		...
		if (mesh) {
			...
			if (type == GT_JET) {
				...
				else if (op1 == 0 || op1 >= 3) g_PlayerData[n].Jet[g_PlayerData[n].ChipCount] = i;
			}
			if (type != GT_COWL) {
				float r1 = ((((int)chip->color) >> 10) / 32.0f)*w2 + ((((int)chip2->color) >> 10) / 32.0f)*w1;
				float g1 = (((((int)chip->color) & 0x3e0) >> 5) / 32.0f)*w2 + (((((int)chip2->color) & 0x3e0) >> 5) / 32.0f)*w1;
...
//    InitAudio( m_hWnd );
	...
	g_World = new GWorld(1 / (float)s_LimitFPS, GDTSTEP);	//ステップ・サブステップ
...
VOID CMyD3DApplication::ReadSettings()
{
		...
		0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hkey, NULL))
	{
		...
		if (m_dLimidFPS == (1000 / 15)) s_LimitFPS = 15;
		else  s_LimitFPS = 30;
...
HRESULT CMyD3DApplication::_send_network_bullet(DWORD t, bool moveEnd)
{
	...
	if (g_DPlay->GetNumPlayers() > 0 && t - m_lastT2 > (DWORD)GNETSPAN / 3 && moveEnd) {
		...
		if (j > 0) {
			...
			g_DPlay->SendTo(DPNID_ALL_PLAYERS_GROUP, (BYTE*)&stream, mySize, 300, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE);
		}
	}
	return S_OK;
}
	...
	if (g_DPlay->GetNumPlayers() > 0 && t - m_lastT2 > (DWORD)GNETSPAN / 3 && moveEnd) {
		...
		if (g_World->B26Bullet) {
			...
			if (j > 0) {
				...
				g_DPlay->SendTo(DPNID_ALL_PLAYERS_GROUP, (BYTE*)&stream, mySize, GNETSPAN / 3, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE | DPNSEND_PRIORITY_LOW);
			}
		}
	...
	if (g_DPlay->GetNumPlayers() > 0 && t - m_lastT > (DWORD)GNETSPAN && moveEnd) {
		if (strlen(s_MessageData)) {
			...
			*((int*)stream.data) = scenarioCode;
			strcpy((char*)&stream.data[4], s_MessageData);
			DWORD size = (DWORD)(sizeof(int) + strlen(s_MessageData) + 1 + sizeof(short));
			g_DPlay->SendTo(DPNID_ALL_PLAYERS_GROUP,(BYTE*)&stream, size, GNETSPAN / 2, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE);
			s_MessageData[0] = '\0';
		}
	}
	return S_OK;
...
HRESULT CMyD3DApplication::_send_network_model_full(DWORD t, bool moveEnd)
{
	...
	if (g_DPlay->GetNumPlayers() > 0 && t - m_lastT > (DWORD)GNETSPAN && moveEnd) {
		...
		g_DPlay->SendTo(DPNID_ALL_PLAYERS_GROUP,(BYTE*)&stream, MyNetDataSize, GNETSPAN / 2, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE | DPNSEND_PRIORITY_LOW);
	}
	return S_OK;
}
HRESULT CMyD3DApplication::_send_network_model_short(DWORD t, bool moveEnd)
{
	...
	if (g_DPlay->GetNumPlayers()>0 && t - m_lastT2 > (DWORD)GNETSPAN / 3 && moveEnd) {
		...
		g_DPlay->SendTo(DPNID_ALL_PLAYERS_GROUP,(BYTE*)&stream, MyNetDataSize, GNETSPAN / 3, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE | DPNSEND_PRIORITY_LOW);
	}
	return S_OK;
}
		...
		if (ControlKeysLock[2] == false) {
			...
			if (GetOpenFileName(&ofn)) {
				...
				if (g_DPlay->GetNumPlayers() > 0) {	//初期化を送信する
					...
					g_DPlay->SendTo(DPNID_ALL_PLAYERS_GROUP,(BYTE*)&stream, sz, 60, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE);
				}
			}
			if (win == FALSE)
			{
			...
			for (j = 0; j < g_KeyList[i].Count; j++) {
				if (g_KeyList[i].ValNo[j] >= 0) {
					if (g_KeyList[i].ResetFlag[j]) {
						...
						val += g_KeyList[i].Step[j] * 30.0f / s_LimitFPS;
					}
				...
				if (g_KeyList[i].ValNo[j] >= 0) {
					if (g_KeyList[i].ReleaseFlag[j]) {
						if (g_KeyList[i].ResetFlag2[j]) {
							...
							val = g_ValList[g_KeyList[i].ValNo[j]].Val;
							val += g_KeyList[i].Step2[j] * 30.0f / s_LimitFPS;
						}
						if (val > g_ValList[g_KeyList[i].ValNo[j]].Max) val = g_ValList[g_KeyList[i].ValNo[j]].Max;
						if (val < g_ValList[g_KeyList[i].ValNo[j]].Min) val = g_ValList[g_KeyList[i].ValNo[j]].Min;
						g_ValList[g_KeyList[i].ValNo[j]].Val = val;
			...
			if (g_Chip[i]->Top != NULL) {
				...
				if (TorqueFlag && (g_Chip[i]->MeshNo == MESH_WHEEL || g_Chip[i]->MeshNo == MESH_RLW) && g_Chip[i]->Power != 0) {
					if (!EfficientFlag) {
						...
						g_Chip[i]->Top->UseFuel(f*30.0 / s_LimitFPS);
				...
				//ARM弾発射
					...
					GBulletVertex *bul = g_Bullet->Add(g_Chip[i], g_Chip[i]->X, d*as*30.0f / (GFloat)s_LimitFPS + g_Chip[i]->V*g_Chip[i]->World->Dt*(GFloat)GDTSTEP, g_Chip[i]->ArmEnergy, (GFloat)f*0.3f, dist, p, -1);
				...
				//JET
					...
					if (g_Chip[i]->Option == 1) {
						...
						if (JetFlag) s_TotalPower += (GFloat)fabs(po);
					}
...
HRESULT CMyD3DApplication::Render()
{
	...
	if (SUCCEEDED(m_pd3dDevice->BeginScene()))
	{
		...
		// TODO: update world
		s_FPS = m_fFPS;
		g_MyPlayerData.base_fps = (s_LimitFPS << 16) + (int)(m_fFPS + 0.5);
...
HRESULT CMyD3DApplication::RenderText()
{
	...
	if (ShowMeter) {
		...
		float power = (float)s_TotalPower;
	...
	LPARAM lParam)
{
	...
	//s_FPSの再設定
//			if(m_appMax>1) s_LimitFPS=15;else s_LimitFPS=30;
	...
	case WM_SIZE:
		if (wParam == SIZE_MINIMIZED) {
			//			GCHIPSTREAM stream;
			//			stream.code=-1;//CLEAR
			//			if(g_DPlay)	g_DPlay->SendTo(DPNID_ALL_PLAYERS_GROUP,(BYTE*)&stream,sizeof(short));
	...
	case WM_COMMAND:
	{
		...
		case IDM_LIMIT30:
		{
			HMENU hMenu = GetMenu(hWnd);
			s_LimitFPS = 30;
			m_dLimidFPS = (m_dLimidFPS == (1000 / s_LimitFPS)) ? 0L : (1000 / s_LimitFPS);
			if (g_World) g_World->SetStepTime(1.0f / s_LimitFPS);
			if (m_dLimidFPS == (1000 / s_LimitFPS)) {
				CheckMenuItem(hMenu, IDM_LIMIT30, MF_CHECKED);
				CheckMenuItem(hMenu, IDM_LIMIT15, MF_UNCHECKED);
			}
			else {
...
{
	return s_RecieaveMessageCode[playerNumber];
}
void ClearReceiveMessage(int playerNumber)
{
	s_RecieaveMessageData[playerNumber][0] = '\0';
}
